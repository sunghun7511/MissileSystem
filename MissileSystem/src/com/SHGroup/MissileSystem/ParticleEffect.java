...
import com.SHGroup.MissileSystem.ReflectionUtils.PackageType;

public enum ParticleEffect {
	EXPLOSION_NORMAL(fieload("EXPLOSION_NORMAL"), "explode", 0, -1,
			ParticleProperty.DIRECTIONAL), EXPLOSION_LARGE(
			fieload("EXPLOSION_LARGE"), "largeexplode", 1, -1), EXPLOSION_HUGE(
			fieload("EXPLOSION_HUGE"), "hugeexplosion", 2, -1), FIREWORKS_SPARK(
			fieload("FIREWORKS_SPARK"), "fireworksSpark", 3, -1,
			ParticleProperty.DIRECTIONAL), WATER_BUBBLE(
			fieload("WATER_BUBBLE"), "bubble", 4, -1,
			ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_WATER), WATER_SPLASH(
			fieload("WATER_SPLASH"), "splash", 5, -1,
			ParticleProperty.DIRECTIONAL), WATER_WAKE(fieload("WATER_WAKE"),
			"wake", 6, 7, ParticleProperty.DIRECTIONAL), SUSPENDED(
			fieload("SUSPENDED"), "suspended", 7, -1,
			ParticleProperty.REQUIRES_WATER), SUSPENDED_DEPTH(
			fieload("SUSPENDED_DEPTH"), "depthSuspend", 8, -1,
			ParticleProperty.DIRECTIONAL), CRIT(fieload("CRIT"), "crit", 9, -1,
			ParticleProperty.DIRECTIONAL), CRIT_MAGIC(fieload("CRIT_MAGIC"),
			"magicCrit", 10, -1, ParticleProperty.DIRECTIONAL), SMOKE_NORMAL(
			fieload("SMOKE_NORMAL"), "smoke", 11, -1,
			ParticleProperty.DIRECTIONAL), SMOKE_LARGE(fieload("SMOKE_LARGE"),
			"largesmoke", 12, -1, ParticleProperty.DIRECTIONAL), SPELL(
			fieload("SPELL"), "spell", 13, -1), SPELL_INSTANT(
			fieload("SPELL_INSTANT"), "instantSpell", 14, -1), SPELL_MOB(
			fieload("SPELL_MOB"), "mobSpell", 15, -1,
			ParticleProperty.COLORABLE), SPELL_MOB_AMBIENT(
			fieload("SPELL_MOB_AMBIENT"), "mobSpellAmbient", 16, -1,
			ParticleProperty.COLORABLE), SPELL_WITCH(fieload("SPELL_WITCH"),
			"witchMagic", 17, -1), DRIP_WATER(fieload("DRIP_WATER"),
			"dripWater", 18, -1), DRIP_LAVA(fieload("DRIP_LAVA"), "dripLava",
			19, -1), VILLAGER_ANGRY(fieload("VILLAGER_ANGRY"), "angryVillager",
			20, -1), VILLAGER_HAPPY(fieload("VILLAGER_HAPPY"), "happyVillager",
			21, -1, ParticleProperty.DIRECTIONAL), TOWN_AURA(
			fieload("TOWN_AURA"), "townaura", 22, -1,
			ParticleProperty.DIRECTIONAL), NOTE(fieload("NOTE"), "note", 23,
			-1, ParticleProperty.COLORABLE), PORTAL(fieload("PORTAL"),
			"portal", 24, -1, ParticleProperty.DIRECTIONAL), ENCHANTMENT_TABLE(
			fieload("ENCHANTMENT_TABLE"), "enchantmenttable", 25, -1,
			ParticleProperty.DIRECTIONAL), FLAME(fieload("FLAME"), "flame", 26,
			-1, ParticleProperty.DIRECTIONAL), LAVA(fieload("LAVA"), "lava",
			27, -1), FOOTSTEP(fieload("FOOTSTEP"), "footstep", 28, -1), CLOUD(
			fieload("CLOUD"), "cloud", 29, -1, ParticleProperty.DIRECTIONAL), REDSTONE(
			fieload("REDSTONE"), "reddust", 30, -1, ParticleProperty.COLORABLE), SNOWBALL(
			fieload("SNOWBALL"), "snowballpoof", 31, -1), SNOW_SHOVEL(
			fieload("SNOW_SHOVEL"), "snowshovel", 32, -1,
			ParticleProperty.DIRECTIONAL), SLIME(fieload("SLIME"), "slime", 33,
			-1), HEART(fieload("HEART"), "heart", 34, -1), BARRIER(
			fieload("BARRIER"), "barrier", 35, 8), ITEM_CRACK(
			fieload("ITEM_CRACK"), "iconcrack", 36, -1,
			ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA), BLOCK_CRACK(
			fieload("BLOCK_CRACK"), "blockcrack", 37, -1,
			ParticleProperty.REQUIRES_DATA), BLOCK_DUST(fieload("BLOCK_DUST"),
			"blockdust", 38, 7, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_DATA), WATER_DROP(fieload("WATER_DROP"),
			"droplet", 39, 8), ITEM_TAKE(fieload("ITEM_TAKE"), "take", 40, 8), MOB_APPEARANCE(
			fieload("MOB_APPEARANCE"), "mobappearance", 41, 8);
	...
	private ParticleEffect(String name, String realname, int id,
			int requiredVersion, ParticleProperty... properties) {
		this.name = name;
	...
	private static String fieload(String string) {
		try {
			...
			if (data.containsKey(string)) {
				return data.get(string).replace("짠", "&").replace(" ", "_");
			} else {
				all += (all.endsWith("\n") ? "" : "\r\n") + string + ":"
						+ string;
	...
	public void display(float offsetX, float offsetY, float offsetZ,
			...
			ParticleColor c, ParticleData pd) throws ParticleVersionException,
		...
		if (hasProperty(ParticleProperty.REQUIRES_DATA)) {
			if (pd != null) {
				display(pd, center.getDirection(), speed, center, range);
			} else {
				return;
			}
		}
		...
		if (hasProperty(ParticleProperty.COLORABLE)
				&& Integer.parseInt(Character.toString(PackageType
						.getServerVersion().charAt(3))) > 6) {
			if (this == NOTE) {
				if (c != null) {
					display(c, center, range);
					return;
				}
				Random r = new Random();
				int color = r.nextInt(24);
				display(new NoteColor(color), center, range);
				return;
				...
				Random r = new Random();
				display(new OrdinaryColor(r.nextInt(256), r.nextInt(256),
						r.nextInt(256)), center, range);
			}
			return;
		}
					...
					if (data != null) {
						...
						ReflectionUtils
								.setValue(
										...
										true,
										"k",
										effect == ParticleEffect.ITEM_CRACK ? packetData
												: new int[] { packetData[0]
														| (packetData[1] << 12) });
					}
				}
